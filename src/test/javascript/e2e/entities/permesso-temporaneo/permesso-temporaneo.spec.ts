...
describe('PermessoTemporaneo e2e test', () => {
  ...
  it('should load PermessoTemporaneos', async () => {
    ...
    expect(await permessoTemporaneoComponentsPage.getTitle()).to.eq('ztlApp.permessoTemporaneo.home.title');
    await browser.wait(
  ...
  it('should load create PermessoTemporaneo page', async () => {
    ...
    expect(await permessoTemporaneoUpdatePage.getPageTitle()).to.eq('ztlApp.permessoTemporaneo.home.createOrEditLabel');
  ...
  it('should create and save PermessoTemporaneos', async () => {
    ...
    const selectedTargaEstera = permessoTemporaneoUpdatePage.getTargaEsteraInput();
    if (await selectedTargaEstera.isSelected()) {
      await permessoTemporaneoUpdatePage.getTargaEsteraInput().click();
      expect(await permessoTemporaneoUpdatePage.getTargaEsteraInput().isSelected(), 'Expected targaEstera not to be selected').to.be.false;
    } else {
      await permessoTemporaneoUpdatePage.getTargaEsteraInput().click();
      expect(await permessoTemporaneoUpdatePage.getTargaEsteraInput().isSelected(), 'Expected targaEstera to be selected').to.be.true;
    }
  ...
  it('should delete last PermessoTemporaneo', async () => {
    ...
    expect(await permessoTemporaneoDeleteDialog.getDialogTitle()).to.eq('ztlApp.permessoTemporaneo.delete.question');
